...
CSVParser.prototype._transform = function(chunk, encoding, done) {
  //<co id="callout-streams-transform-2" />
  let c
  let i
  ...
  for (i = 0; i < chunk.length; i++) {
    //<co id="callout-streams-transform-3" />
    c = chunk.charAt(i)

    if (c === ',') {
      //<co id="callout-streams-transform-4" />
      this.addValue()
    } else if (c === '\n') {
      this.addValue()
      if (this.line > 0) {
...
CSVParser.prototype.toObject = function() {
  ...
  for (i = 0; i < this.headers.length; i++) {
    //<co id="callout-streams-transform-7" />
...
CSVParser.prototype.addValue = function() {
  if (this.line === 0) {
    //<co id="callout-streams-transform-8" />
